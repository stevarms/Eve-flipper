...
| `ui/OpenMarketButton.tsx` + `ui/CopyButton.tsx` + `ui/ItemRef.tsx` | The two row actions and the icon+name+actions composite. Before these there were five open-in-EVE treatments (a 🎮 emoji, `ExternalLink`+"List", `ExternalLink`+"Open market", bare text, and context-menu-only) and four clipboard glyphs (📋, ⧉, ⎘, lucide `Copy`) with two confirmation models. Both are **icon-only and cannot render text** — that is the property that stops a sixth variant appearing. Visibility is a prop, not a per-call-site decision: `reveal="always"` on tables whose purpose is acting in-game, `reveal="hover"` on reference surfaces. |
