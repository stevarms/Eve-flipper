...
func TestHostedQuotaFeatureMappingCoversHeavyHostedPosts(t *testing.T) {
	tests := []struct {
		...
		{http.MethodPost, "/api/pi/factory-plan", "scans"},
		...
		{http.MethodPost, "/api/auth/industry/projects/42/materials/recalc-remaining", "scans"},
...
func TestHostedQuotaFeatureMappingClassifiesAllPostAPIRoutes(t *testing.T) {
	...
	intentionallyUnmetered := map[string]string{
		...
		"/api/presets":                                   "saved-preset CRUD",
		"/api/presets/{presetID}/activate":               "saved-preset CRUD",
		...
		"/api/auth/today/state":                          "marks one action done or skipped, no ESI",
		"/api/auth/fw/campaigns":                         "FW campaign CRUD",
		"/api/auth/fw/campaigns/{id}/lots":               "FW lot CRUD",
		"/api/auth/fw/campaigns/{id}/lots/{lotID}/state": "FW lot state transition, no ESI",
		// The bulk form of the line above and no more expensive: one transaction of
		// database writes, no ESI. Metering it would price moving fourteen lots at
		// fourteen times moving one, which is the opposite of why it exists.
		"/api/auth/fw/campaigns/{id}/lots/bulk": "FW lot state transition in bulk, no ESI",
		// The desk fetch behind this is the same one GET /api/auth/order-desk
		// does unmetered; the POST only writes the fills that fetch proved.
		"/api/auth/fw/campaigns/{id}/orders/apply": "FW lot bookkeeping over an unmetered desk build",
